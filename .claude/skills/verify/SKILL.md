...
cd stock-api && ./stock-api.exe         # serves API + static on :8080
```
Port 8080 often already has a running instance (bind error = fine): `http.FileServer` serves `./static` from disk, so a fresh `npm run build` is live immediately — no server restart needed for UI-only changes. Backend changes DO need rebuild (`go build -o stock-api.exe .`) + restart.
...
Navigation flow:
1. `goto http://localhost:8080` → lands on Watchlist view.
