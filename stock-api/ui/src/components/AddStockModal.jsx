  ...
  return (
    ...
    >
      <div
        ...
        className="bg-tv-card border border-tv-border rounded-2xl w-[400px] max-w-[95vw] shadow-2xl"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-tv-border">
          <h3 id="add-stock-title" className="font-bold text-base">Tambah Saham IDX</h3>
          <button
            type="button"
            onClick={onClose}
            aria-label="Tutup"
            className="text-tv-muted hover:text-tv-text transition-colors text-xl leading-none"
          >
            ×
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            <label htmlFor="add-symbol" className="block text-xs font-semibold text-tv-muted uppercase tracking-wider mb-2">
              Kode Saham
            </label>
            <input
              id="add-symbol"
              ...
              placeholder="cth: BBCA · TLKM · GOTO"
              autoFocus
          ...
          <div>
            <label htmlFor="add-from" className="block text-xs font-semibold text-tv-muted uppercase tracking-wider mb-2">
          ...
          {error && (
            <div className="bg-tv-red/10 border border-tv-red/30 text-tv-red text-xs px-4 py-3 rounded-lg" role="alert">
              {error}
            </div>
          )}
